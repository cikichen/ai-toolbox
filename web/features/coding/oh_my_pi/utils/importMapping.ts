...
export interface OmpCcSwitchImport {
  providerKey: string;
...
/**
 * Convert a CC Switch Claude candidate into an Oh My Pi provider import record. OMP stores the
 * API key inline on the provider config (`apiKey` field), so the candidate's Anthropic env vars
 * map straight onto the modelsProvider. Returns null when the candidate carries neither base URL
 * nor key.
 */
export const extractOmpProviderFromCcSwitch = (
  candidate: CcSwitchProviderCandidate,
): OmpCcSwitchImport | null => {
