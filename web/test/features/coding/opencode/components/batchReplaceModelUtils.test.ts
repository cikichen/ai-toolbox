...
test('collectBatchReplaceSourceUsage includes fallback models in source candidates', () => {
  const result = collectBatchReplaceSourceUsage({
    values: {
      ...
      agent_oracle_reasoning: 'high',
      agent_oracle_models: ['qwen3.5', 'gpt-4.1'],
    },
    modelFieldNames: ['agent_oracle_model'],
    getVariantFieldName: (modelFieldName) => modelFieldName.replace('_model', '_reasoning'),
    getFallbackFieldName: (modelFieldName) => modelFieldName.replace('_model', '_models'),
  });
...
test('applyBatchReplaceModel replaces fallback-only matches when no source variant is selected', () => {
  ...
  assert.deepEqual(result, {
    updateValues: {
      agent_oracle_models: ['qwen3.7', 'gpt-4.1'],
      agent_reviewer_models: ['qwen3.7'],
    },
    replacedCount: 2,
...
test('applyBatchReplaceModel does not replace fallback models when source variant filtering is enabled', () => {
  const result = applyBatchReplaceModel({
    values: {
      agent_oracle_model: 'openai/gpt-5.4',
      agent_oracle_reasoning: 'high',
      agent_oracle_models: ['openai/gpt-5.4'],
    },
    modelFieldNames: ['agent_oracle_model'],
...
test('applyBatchReplaceModel supports openagent field naming for category fallback models', () => {
  const result = applyBatchReplaceModel({
    values: {
      ...
      agent_architect_models: ['qwen3.5'],
      category_coding: 'anthropic/claude-sonnet-4.5',
      category_coding_models: ['qwen3.5', 'gpt-4.1'],
    },
    modelFieldNames: ['agent_architect', 'category_coding'],
    fromModel: 'qwen3.5',
    toModel: 'qwen3.7',
    targetVariants: [],
    getVariantFieldName: (modelFieldName) => `${modelFieldName}_reasoning`,
    getFallbackFieldName: (modelFieldName) => `${modelFieldName}_models`,
  });

  assert.deepEqual(result, {
    updateValues: {
      agent_architect_models: ['qwen3.7'],
      category_coding_models: ['qwen3.7', 'gpt-4.1'],
    },
    replacedCount: 2,
    clearedVariantCount: 0,
  });
