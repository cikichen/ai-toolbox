...
- Official OAuth freshness: apply / limits already call `ensure_fresh_auth_snapshot` (lead 5 minutes). Startup / interval passes are scheduled by `coding::auth_refresh` via `refresh_applied_gemini_cli_accounts_if_needed` (all persisted non-virtual OAuth accounts, default interval 15m). Non-applied accounts still refresh tokens into SQLite; only applied accounts rewrite live `oauth_creds.json`. Background token refresh must **preserve** stored quota/limit fields on the account row (do not rebuild content with `quota_snapshot=None` and wipe weekly limits). On runtime write, emit `wsl-sync-request-geminicli` like apply. Quota (`retrieveUserQuota`) is not scheduled by auth_refresh.
