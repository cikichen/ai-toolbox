...
- OMOS Council 合成模型应写 `agents.council`，不要写 `council.master` / `master_timeout` / `master_fallback`。应用配置时必须先对 raw `council` 做 legacy master promote，并把 `master_fallback` 按顺序、按 model id 去重后追加到 `agents.council.model` 数组，再 `normalize` 剔除 master 字段；已有 `agents.council` 保留其主模型和未知字段，但仍要吸收 legacy fallback。顺序反了会让迁移变成死代码。profile agents 覆盖 global/base agents 时，若 profile 未定义 `agents.council` 则保留 base 中的 synthesizer。
