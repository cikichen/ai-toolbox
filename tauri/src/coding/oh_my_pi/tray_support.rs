...
use super::types::OmpProviderWarning;
...
pub async fn get_omp_tray_data<R: Runtime>(app: &AppHandle<R>) -> Result<TrayModelData, String> {
    let runtime_config = super::commands::read_omp_runtime_config(app.state()).await?;
    ...
    for provider in runtime_config.providers {
        let is_provider_missing = provider
            .warnings
            .iter()
            .any(|warning| matches!(warning, OmpProviderWarning::MissingProvider));
...
pub async fn apply_omp_model<R: Runtime>(
    app: &AppHandle<R>,
    provider_key: &str,
    model_id: &str,
) -> Result<(), String> {
    ...
    let db = state.db();
    super::commands::apply_omp_default_model_internal(&db, app, provider_key, model_id, true).await
}
...
pub async fn get_omp_prompt_tray_data<R: Runtime>(
    app: &AppHandle<R>,
...
pub async fn apply_omp_prompt_config<R: Runtime>(
    app: &AppHandle<R>,
    config_id: &str,
) -> Result<(), String> {
    super::commands::apply_omp_prompt_config_internal(app.state(), app, config_id, true).await
}
