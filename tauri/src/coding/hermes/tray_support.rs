...
use super::types::HermesProviderWarning;
...
pub async fn get_hermes_tray_data<R: Runtime>(app: &AppHandle<R>) -> Result<TrayModelData, String> {
    let runtime_config = super::commands::read_hermes_runtime_config(app.state()).await?;
    ...
    for provider in runtime_config.providers {
        let is_provider_missing = provider
            .warnings
            .iter()
            .any(|warning| matches!(warning, HermesProviderWarning::MissingProvider));
...
pub async fn apply_hermes_model<R: Runtime>(
    app: &AppHandle<R>,
    provider_key: &str,
    model_id: &str,
) -> Result<(), String> {
    ...
    let db = state.db();
    super::commands::apply_hermes_model_internal(&db, app, provider_key, model_id, true).await
}
...
pub async fn get_hermes_prompt_tray_data<R: Runtime>(
    app: &AppHandle<R>,
) -> Result<TrayPromptData, String> {
    let configs = super::commands::list_hermes_prompt_configs(app.state()).await?;
    let items = configs
        .into_iter()
...
pub async fn apply_hermes_prompt_config<R: Runtime>(
    app: &AppHandle<R>,
    config_id: &str,
) -> Result<(), String> {
    super::commands::apply_hermes_prompt_config_internal(app.state(), app, config_id, true).await
}
